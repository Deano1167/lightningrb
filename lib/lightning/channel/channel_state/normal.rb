    ...
    class ChannelState
      class Normal < ChannelState
        def next(message, data)
          ...
          when UpdateAddHtlc
            new_commitments, new_message = Commitment.receive_add(data[:commitments], message, context.spv)
          ...
          when CommandSignature
            ...
            when WaitingForRevocation
              ...
              new_commitments[:local_changes][:signed].each do |change|
                context.relayer << CommandAck[update.channel_id, update.id] if change.is_a?(UpdateFulfillHtlc)
              end
